...
<html>
  <head>
    ...
    function check1() {
      ...
      var intro = document.forms['form2']['intro'].value;
      var ok = true;
      if (name == null || name == "") {
      ...
      if (intro == null || intro== "") {
          $("#introstr").text("required, please enter a intro");
          ok = false;
      } else {
          $("#introstr").text("");
      }
  ...
  <body>
    ...
    ?>
    <div class='col-xs-12'><h2 style="text-align:center">- Edit Rosters -</h2></div>
    <?php
    ...
    <?php
      ...
      if(isset($_POST['submit3'])) {
        ...
        $post_intro = filter_input( INPUT_POST, 'intro', FILTER_SANITIZE_STRING );
        
         
        $query = $query = "INSERT INTO Roster (name, intro) VALUES ('$post_name', '$post_intro'); ";
        $result = $mysqli->query($query);
        if ( $result ) {
          header("Location: roster.php?mssg=2");
          exit();
        } else {
        ...
        $post_intro = filter_input( INPUT_POST, 'intro', FILTER_SANITIZE_STRING );
        $posted_intro = "intro = '$post_intro'";
        if ($post_intro == null || $post_intro == "") {
          $posted_intro = "";
        }

        if ($posted_intro == "" && $posted_name == "") {
          echo '<p class="query1">You did not enter any new data</p>';
        } else {
          $query = "";
          if ($posted_intro != "" && $posted_name != "") {
            $query = "UPDATE Roster SET $posted_name, $posted_intro WHERE id = $newid; ";
          } else if ($posted_intro != "") {
            $query = "UPDATE Roster SET $posted_intro WHERE id = $newid; ";
          } else {
            $query = "UPDATE Roster SET $posted_name WHERE id = $newid; ";
          }
          
          //echo $query;
          $result = $mysqli->query($query);
          if ( $result ) {
            header("Location: roster.php?mssg=1");
          } else {
        ...
        $newid = filter_input( INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT );
        $query = "UPDATE Image SET forRoster = NULL WHERE forRoster = $newid; ";
        $result = $mysqli->query($query);
        if ($result) {
          $query = "DELETE FROM Roster WHERE id = $newid; ";
          $result = $mysqli->query($query);
          if ($result) {
            header("Location: roster.php?mssg=3");
          }
        } else {
      ...
      if (isset($_GET['id'])) {
      	$newid = filter_input( INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT );
      	$query2 = "SELECT * FROM Roster WHERE id = $newid;";
      	$result2 = $mysqli->query($query2);
      	if ($result2 && $result2->num_rows >= 1) {
      		$row = $result2->fetch_assoc();
          echo "<p class='query1'>ID: {$row['id']}</p><br>";
      		echo "<p class='query1'>Name: {$row['name']}</p><br>";
      		echo "<p class='query1'>Intro: {$row['intro']}</p><br>";
      		echo "<hr>";
          print("<form name='form5' class='formBox' action='roster.php?id=$newid' method='POST' onsubmit='return check2();'>"); 
          print("<input type='submit' class='submitButton' name='submit5' value='Delete this Entry'/>"); 
          print "</form>";
          echo "<hr>";
	        print("<form name='form1' class='formBox' action='roster.php?id=$newid' method='POST'>"); 
	        print("New Name: <span>Put no value if not changing this field</span><input type='text' class='input' name='name'> <br>");
	        print("New Intro: <span>Put no value if not changing this field</span><input type='text' class='input' name='intro'> <br>");
	      ...
	      $query1 = "SELECT id FROM Roster;";
	      $result = $mysqli->query($query1);
	      if ($result && $result->num_rows >= 1) {
	      	print("<h2 style='text-align:center'>Edit Existing Roster</h2>");
	        print("<form name='form1' class='formBox' action='roster.php' method='GET'>"); 
	        print "<div class='middle'>Select roster: <select name='id'>";
	      ...
	      print("<h2 style='text-align:center'>Add new Roster entry</h2>");   
	        print("<form name='form2' class='formBox' action='roster.php' method='POST' onsubmit='return check1();'>"); 
	        print("Name: <span id='namestr'></span><input type='text' class='input' name='name'> <br>");
	        print("Intro: <span id='introstr'></span><input type='text' class='input' name='intro'> <br>");
