...
<html>
  <head>
    ...
    function check1() {
      ...
      if (url == null || url == "") {
          $("#urlstr").text("required, please enter a url");
          ok = false;
      } else {
          $("#urlstr").text("");
      }
  ...
  <body>
    ...
    <div class='col-xs-12'><h2 style="text-align:center">- Edit Videos -</h2></div>
    <?php
    ...
    <?php
      ...
      if(isset($_POST['submit3'])) {
        ...
        $post_url = filter_input( INPUT_POST, 'url', FILTER_SANITIZE_STRING );
        ...
        // } else {
          ...
          $query = $query = "INSERT INTO Video (url, text, credit, dateTaken, forEvent) VALUES ('$post_url', '$post_text', '$post_credit', '$post_time', $post_event); ";
          if ($post_event == 0) {
            $query = $query = "INSERT INTO Video (url, text, credit, dateTaken, forEvent) VALUES ('$post_url', '$post_text', '$post_credit', '$post_time', NULL); ";
          
          }
          //echo $query;
          $result = $mysqli->query($query);
          if ( $result ) {
            header("Location: video.php?mssg=2");
          } else {
        ...
        $query = "UPDATE Video SET $posted_text $posted_credit forEvent=$post_event WHERE id = $newid; ";

        //echo $query;
        $result = $mysqli->query($query);
        if ( $result ) {
          header("Location: video.php?mssg=1");
        } else {
          echo '<p class="query1">You did not update this data entry successfully.</p>';
        }
      } elseif (isset($_POST['submit5'])) {
        $newid = filter_input( INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT );
        $query = "DELETE FROM VideoCategory WHERE videoid = $newid; ";
        $result = $mysqli->query($query);
        if ( $result ) {
          $query = "DELETE FROM Video WHERE id = $newid; ";
          $result = $mysqli->query($query);
          if ($result) {
            header("Location: video.php?mssg=3");
          } else {
      ...
      if (isset($_GET['id'])) {
      	$newid = filter_input( INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT );
      	$query2 = "SELECT * FROM Video WHERE id = $newid;";
      	...
      	if ($result2 && $result2->num_rows >= 1) {
      		...
      		echo "<hr>";
          print("<form name='form5' class='formBox' action='video.php?id=$newid' method='POST' onsubmit='return check2();'>"); 
          print("<input type='submit' class='submitButton' name='submit5' value='Delete this Entry'/>"); 
          print "</form>";
          echo "<hr>";
	        print("<form name='form1' class='formBox' action='video.php?id=$newid' method='POST'>"); 
	      ...
	      $query1 = "SELECT id FROM Video;";
	      $result = $mysqli->query($query1);
	      if ($result && $result->num_rows >= 1) {
	      	print("<h2 style='text-align:center'>Edit Existing Video</h2>");
	        print("<form name='form1' class='formBox' action='video.php' method='GET'>"); 
	        print "<div class='middle'>Select Video: <select name='id'>";
	        while ( $row = $result->fetch_assoc()) {
          ...
          echo "<hr>";
	        ...
	        print("<h2 style='text-align:center'>Add new Video entry</h2>");   
	        print("<form name='form2' class='formBox' action='video.php' method='POST' onsubmit='return check1();'>"); 
          ...
          print("</select><br>");
	        print("URL: <span id='urlstr'></span><input type='text' class='input' name='url'> <br>");
